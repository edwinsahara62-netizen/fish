...
async function fetchImageFromPexels(keyword, pexelsApiKey) {
  ...
  try {
    console.log(` -> Step 6: Searching for image with keyword: "${keyword}"`);
...
async function main() {
  ...
  try {
    ...
    let keywordsToGenerate = keywords.filter(keyword => !existingSlugs.has(createSlug(keyword)));
    ...
    if (keywordsToGenerate.length > 0) {
      ...
      for (let i = 0; i < keywordsToGenerate.length; i++) {
        ...
        keywordCount++;
        const currentKeyword = keywordsToGenerate[i].trim();
        ...
        try {
          ...
          jsonResult.deepDive = await uniquenessBooster(jsonResult.deepDive, analysis.persona, models, currentApiKeyIndex);
          jsonResult.importance = await uniquenessBooster(jsonResult.importance, analysis.persona, models, currentApiKeyIndex);

          await delay(INTERNAL_DELAY_MS);
          
          const existingTitlesForLinking = allArticles.map(a => ({ term: a.term, slug: a.slug }));
          let combinedTextForLinking = `${jsonResult.deepDive}\n\n${jsonResult.importance}`;
          ...
          console.log(` -> SUCCESS: Final article for "${currentKeyword}" is complete and saved.`);
        } catch (e) {
          console.error(` -> FAILED: Skipping keyword "${currentKeyword}". Error: ${e.message}`);
        }
    ...
    if (needsSave) {
      await fs.writeFile(OUTPUT_PATH, JSON.stringify(allArticles, null, 2));
      console.log(`\n--- Image Update Complete. File saved. ---`);
    } else {
